 ...
 * @param {*} graphics 
 */
function Metro( canvas, graphics ) {
    var state = {
        N: graphics ? graphics.sites.length : 30, // quantity of polygons
        SIGN: Math.random() < 0.5 ? -1 : 1, // make positive or negative 1
        vertices: graphics ? graphics.vertices : [],
        isWarpMode: false,
        ...
        canvas: canvas ? canvas : d3.select("canvas").node(),
    ...
    state.graphics = graphics ? graphics : new Graphics();
    console.log("graphics: ", state.graphics);
    /*=====================================================================================================
    ...
    function Graphics() {
        this.SEGMENT = 20;
        this.DISTANCE = 150;
        this.sites = d3.range(state.N).map( () => [Math.random() * state.width(), Math.random() * state.height()] );
        this.voronoi = d3.voronoi().extent([[20, 20], [state.width() - 20, state.height() - 20]]);
        this.diagram = this.voronoi( this.sites );
        this.edges = this.diagram.edges.map( makeEdges );
    ...
    function makePolygons(diagram, edges) {
        return diagram.cells.map((cell, index) => {
            ...
            let vertices = cell.halfedges.map(i => {
                // i is referencing to index of Array[edge]
                let v = cellHalfedgeStart(cell, edges[i]);
                // let vertex = {};
                polygon.site = { x: cell.site[0], y: cell.site[1], color: null };
                // polygon.meta = new SiteMeta( i, POLYGON_TYPE_COLOR[ polygon.type ] );
                // vertex.x = v[0];
                // vertex.y = v[1];
                state.vertices.push({ x: v[0], y: v[1], index: i });
                return state.vertices.length - 1;
            });
            polygon.index = index;
            polygon.vertices = vertices;
            vertices = convertToVertices(vertices);
            polygon.bounds = bounds(vertices);
            polygon.area = polygonArea(vertices);
            polygon.center = { x: polygonCentroid(vertices).x, y: polygonCentroid(vertices).y };
            polygon.type = getType(polygon);
            polygon.site.color = POLYGON_TYPE_COLOR[ polygon.type ];
            cell.site.data[2] = polygon.site.color;
            return polygon;

            function SiteMeta( index, color ) {
                this.index = index;
                this.color = color;
            }
        });
    ...
    function tick() {
        ...
        if(state.isSingleSelected && state.DRAGGED_SUBJECT){
    ...
    function startSingleDragMode() {
        state.isSingleSelected = true;
    }

    function stopSingleDragMode() {
    ...
    function changeEdges(distance, segment) {
        state.graphics.DISTANCE = distance;
        state.graphics.SEGMENT = segment;
        tick();
    }
    ...
    function menu() {
        ...
        let point = { x: x, y: y };
        let polygon = state.graphics.polygons.filter(poly => polygonContains(convertToVertices(poly.vertices), point))[0];
            ...
            for(let l = 0; l < iterations; l++) {
                for(let k = 0; k < nodes.length; k++) {
                    let node = nodes[k],
                        ...
                        polyPoints = convertToVertices(polygon.vertices),
                    ...
                    for(let j = 0; j < polyPoints.length; j++) {
                        ...
                        let segment1 = { x: polyPoints[j].x, y: polyPoints[j].y };
                        let segment2 = { x: polyPoints[n].x, y: polyPoints[n].y };
                    ...
                    node.orientation = Math.atan2(nearestSegment.p2.y - nearestSegment.p1.y, nearestSegment.p2.x - nearestSegment.p1.x);
                }
            }
            return;
        }
    ...
    function cellHalfedgeStart(cell, edge) {
        if(edge.left.data !== cell.site.data) {
            return [edge.endPoint.x, edge.endPoint.y];
        }
        return [edge.startPoint.x, edge.startPoint.y];
    ...
    // convert Array(index) to Array(state.vertices[index])
    function convertToVertices(indices) {
        return indices.map( i => state.vertices[i] );
    }
    /*=====================================================================================================
                                             Drag Functions
    ======================================================================================================*/
    function dragsubject() {
        ...
        if(!state.isWarpMode) {
            ...
            for(let i = 0; i < state.graphics.polygons.length; i ++){
                ...
                isInside = polygonContains(convertToVertices(state.graphics.polygons[i].vertices), point);
                if(isInside){
                    index = i;
                    break;
                }
            ...
            try {
                ...
                if(vertex.x === edge.startPoint.x && vertex.y === edge.startPoint.y) {
                    sbj = state.graphics.edges[vertex.index].startPoint;
                } else if(vertex.x === edge.endPoint.x && vertex.y === edge.endPoint.y) {
                    sbj = state.graphics.edges[vertex.index].endPoint;
                } else {
                    sbj = vertex;
                }
            } catch { }
    ...
    function makePointInside() {
        ...
        let polyPoints = convertToVertices(poly.vertices);
    ...
    //                 let point = { x: x, y: y };
    //                 if(polygonContains(triangle, point)) {
    ...
    function drawVertices() {
        ...
        state.graphics.edges.forEach(e => {
            ...
            state.context().scale(state.transform.k, state.transform.k);
            state.context().beginPath();
            state.context().moveTo(e.startPoint.x, e.startPoint.y);
            state.context().lineTo(e.endPoint.x, e.endPoint.y);
            ...
            state.context().beginPath();
            state.context().moveTo(e.startPoint.x + 8, e.startPoint.y);
            state.context().arc(e.startPoint.x, e.startPoint.y, 8, 0, 2 * Math.PI);
            state.context().lineWidth = 2;
            state.context().strokeStyle = 'red';
            state.context().stroke();
            state.context().fillStyle = '#CBC5B9';
        ...
        for(let i = 0; i < state.graphics.edges.length; i ++) {
            ...
            if(edge && edge.startPoint && edge.endPoint ) {
                if(edge.left && state.graphics.polygons[edge.left.index].children.length !== 0) {
                    drawPath(edge.startPoint, edge.endPoint, width, color);
    ...
    function drawPath(v1, v2, width, color) {
        let count = 0;
        const DISTANCE = state.graphics.DISTANCE || 150;
        const PATH_LENGTH = distance(v1,v2);
        const SEGMENT = state.graphics.SEGMENT || 20;
    ...
    ======================================================================================================*/
    function onKeyDown() {
        if(!state.isWarpMode && d3.event.keyCode === 13) {
            let sites = $('#sites').val();
            if(!sites.isNaN) {
                newGraphics(sites);
            }
    ...
    return {
        ...
        restartSimulations: restartSimulations,
        stopSimulations: stopSimulations,
        startSingleDragMode: startSingleDragMode,
        stopSingleDragMode: stopSingleDragMode,
        startWarpMode: startWarpMode,
        stopWarpMode: stopWarpMode,
