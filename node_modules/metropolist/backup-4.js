...
========================================
    Edge {
        endpoint1: {x, y, vertexIdnex, edgeIndex},
        endpoint2: {x, y, vertexIdnex, edgeIndex},
        left: {x, y, index, data},
        right[optional]: {x, y, index, data}
    }
========================================
    Item {
        width: Int,
...
* @param {*} graphics 
*/
function Metro( canvas, SEGMENT, DISTANCE, sites, edges, polygons, clusters, vertices ) {
  var state = {
      N: 2, // quantity of polygons
  ...
  // state.graphics = graphics ? convertGraphics(graphics) : new Graphics();
  console.log("loading...", state.graphics);
  /*=====================================================================================================
                                       Constructor Functions
  ======================================================================================================*/
  // convert stored Graphics() object to d3 object
  function convertGraphics(edges) {
      return edges.map( convertEdge );
      ...
      function convertEdge(e) {
          ...
          edge[0].edgeIndex = e.startPoint.edgeIndex;
          edge[0].vertexIndex = e.startPoint.vertexIndex;
          edge[1].edgeIndex = e.startPoint.edgeIndex;
          edge[1].vertexIndex = e.startPoint.vertexIndex;
  ...
  function Graphics() {
      this.SEGMENT = SEGMENT ? SEGMENT : 20;
      this.DISTANCE =  DISTANCE ? DISTANCE : 150;
      this.sites = sites ? sites : d3.range(state.N).map( () => [Math.random() * state.width(), Math.random() * state.height()] );
      this.voronoi = d3.voronoi().extent([[50, 50], [state.width() - 50, state.height() - 50]]);
      this.diagram = this.voronoi( this.sites );
      this.edges = edges ? convertGraphics(edges) : this.diagram.edges;
      ...
      this.polygons = polygons ? polygons : makePolygons(this.diagram, this.edges);
      this.clusters = clusters ? clusters : this.polygons.map( makeCluster );
      this.buildings = getBuildings(this.clusters);
      this.vertices = vertices ? vertices : state.vertices;
  }
  ...
  function makePolygons(diagram) {
      ...
      return diagram.cells.map((cell, index) => {
          ...
          console.log("polygon: ", diagram.polygons());
          ...
          let vertices = cell.halfedges.map(i => {
              ...
              // if(!contains(vertex)) 
              // if(vertex === diagram.edges[i][0]) {
              //     diagram.edges[i][0].edgeIndex = i;
              //     diagram.edges[i][0].vertexIndex = state.vertices.length - 1;
              //     console.log("edge: ", diagram.edges[i]);
              // }
              state.vertices.push(vertex);
              vertex.edgeIndex = i;
              vertex.vertexIndex = state.vertices.length - 1;
      ...
      function contains(vertex) {
          // checks whether a vertex already exists
  ...
  function menu() {
      ...
      let point = [x, y];
      let polygon = state.graphics.polygons.filter(poly => d3.polygonContains(convertToVertices(poly.vertices), point))[0];
      return [{
  ...
  function convertToVertices(indexes) {
  ...
  function dragsubject() {
      ...
      if(!state.isWarpMode) {
          ...
          for(let i = 0; i < state.graphics.polygons.length; i ++){
              ...
              isInside = d3.polygonContains(convertToVertices(state.graphics.polygons[i].vertices), point);
              if(isInside){
                  index = i;
                  break;
              }
          ...
          try {
              ...
              let edge = state.graphics.edges[vertex.edgeIndex];
              if(vertex[0] == edge[0][0] && vertex[1] == edge[0][1]) {
                  sbj = state.graphics.edges[vertex.edgeIndex][0];
              } else if(vertex[0] == edge[1][0] && vertex[1] == edge[1][1]) {
                  sbj = state.graphics.edges[vertex.edgeIndex][1];
              }
          } catch {}
      }
  ...
  function dragged() {
      ...
      if(!state.isWarpMode) {
          ...
          if(x > 50 && x < state.width() - 50 && y > 50 && y < state.height() - 50) {
