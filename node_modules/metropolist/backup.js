    ...
    For each polygon: 
        For each segment(edge) E in polygon P
            For each building B in polygon
                1. Calculate vector V between building center and segment (return to E)
                2. Cal the corresponding force F of V
                3. Apply F on every E in P
 */

 /*
    for each edge E in the "road map"
    let VECTORS(E) = a list of vectors related to E
    for each polygons P that is adjacent to E
        for each building B that is in P
            compute the vector V from P to Q where P is the center of B and Q is the closest point on E to P
            let L be the magnitude (length) of V
            change the length of V to be 1/L
            add V to VECTORS(E)

    for each vertex V in the "road map" let Vforce = a Vector with angle = 0 and magnitude = 0

    for each edge E in the "road map"
    let Vsum = the vector sum of VECTORS(E)
    let P1 be one end of E
    let P2 be the other end of E
    
    let P1force = P1force + Vsum/2
    let P2force = P2force + Vsum/2


    for each vertex V in the "road map"
    move V by an amount normalized(Vforce)
*/
function Metro(canvas, sites, clusters) {
  var state = {
      N: sites ? sites.length : 30, // quantity of polygons
      SIGN: Math.random() < 0.5 ? -1 : 1, // make positive or negative 1
      simulations: [],
      isDragSelected: false,
      DRAGGED_SUBJECT: null,
      canvas: canvas || d3.select("#myCanvas"),
  ...
  ======================================================================================================*/
  function Graphics() {
      this.sites = sites ? sites : d3.range(state.N).map(s => [Math.random() * state.width(), Math.random() * state.height()]);
  ...
  function makePolygons(diagram) {
      ...
      return diagram.cells.map( (cell, index) => {
          ...
          polygon.index = index;
          polygon.children = clusters ? clusters[index] : null;
