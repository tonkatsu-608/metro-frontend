...
function Metro( canvas, graphics ) {
    ...
    function tick(dist, segment) {
        ...
        // drawSites();
        // drawCenters('green');
        drawEdges(2, 'black', dist, segment);
    ...
    // draw edges
    function drawEdges(width, color, dist, segment) {
        for(let i = 0; i < state.graphics.edges.length; i ++) {
            ...
            if(edge && edge[0] && edge[1] ) {
                if(edge.left && state.graphics.polygons[edge.left.index].children.length !== 0) {
                    drawPath(edge[0], edge[1], width, color, dist, segment);
    ...
    // draw segment
    function drawPath(v1, v2, width, color, dist, segment) {
        let count = 0;
        const DISTANCE = dist || 150;
        const PATH_LENGTH = distance(v1,v2);
        const SEGMENT = segment || 20;
