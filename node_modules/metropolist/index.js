...
function Metro( canvas, data ) {
    ...
    ======================================================================================================*/
    // function Graphics() {
    //     this.SEGMENT = data ? data.segment : 20;
    //     this.DISTANCE =  data ? data.distance : 150;
    //     this.sites = data ? data.sites : d3.range(state.N).map( () => [Math.random() * state.width(), Math.random() * state.height()] );
    ...
    //         if(e.hasOwnProperty('left')) delete e.left.data;
    //         if(e.hasOwnProperty('right')) delete e.right.data;
    //     });
    ...
    function Graphics() {
        ...
        if(!data) {
            this.edges.forEach(e => {
                if(e === null || e === undefined) return;
                if(e.left !== undefined) delete e.left.data;
                if(e.right !== undefined) delete e.right.data;
            });
        }
        // this.triangles = this.diagram.triangles();
        this.links = data ? data.links : this.diagram.links();
        this.polygons = data ? data.polygons : makePolygons(this.diagram);
    ...
    function makePolygons(diagram) {
        ...
        return diagram.cells.map((cell, index) => {
            ...
            let vertices = cell.halfedges.map(i => {
                ...
                // }
                vertex.edgeIndex = i;
                vertex.vertexIndex = state.vertices.length;
            ...
            vertices = convert2Vertices(vertices);
    ...
    function makeCluster(poly) {
        ...
        poly.children = dots.map(d => d.index);
        return dots;
    ...
    function startSimulations() {
        ...
        state.graphics.polygons.forEach(p => {
            makeSimulations(p, convert2Cluster(p.children));
        });
    ...
    function menu() {
        ...
        let point = [x, y];
        let polygon = state.graphics.polygons.filter(poly => d3.polygonContains(convert2Vertices(poly.vertices), point))[0];
        return [{
    ...
    // remake cluster after changing its parent's type
    function remakeCluster(polygon) {
        polygon.children.forEach( c => state.buildings.splice(c, 1, {}) );
            ...
            for(let l = 0; l < iterations; l++) {
                for(let k = 0; k < nodes.length; k++) {
                    let node = nodes[k],
                        ...
                        polyPoints = convert2Vertices(polygon.vertices),
    ...
    function convert2Cluster(indexes) {
        return indexes.map( i => state.buildings[i] );
    }
    /*=====================================================================================================
                                             Drag Functions
    ======================================================================================================*/
    function dragsubject() {
        ...
        if(!state.isWarpMode) {
            ...
            for(let i = 0; i < state.graphics.polygons.length; i ++){
                ...
                isInside = d3.polygonContains(convert2Vertices(state.graphics.polygons[i].vertices), point);
                if(isInside){
                    index = i;
                    break;
                }
    ...
    function dragged() {
        ...
        if(!state.isWarpMode) {
            ...
            if(x > 50 && x < state.width() - 50 && y > 50 && y < state.height() - 50) {
                ...
                state.vertices[d3.event.subject.vertexIndex][0] = x;
                state.vertices[d3.event.subject.vertexIndex][1] = y;
                drawVertices();
            }
        }
    }
    ...
    function makePointInside() {
        ...
        let polyPoints = convert2Vertices(poly.vertices);
    ...
    function drawVertices() {
        ...
        state.graphics.edges.forEach(e => {
            ...
            if(e[1]) state.context().lineTo(e[1][0], e[1][1]);
