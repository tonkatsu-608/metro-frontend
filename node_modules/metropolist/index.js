...
function Metro(canvas, cursorCanvas, data) {
    ...
    function polygon2Array(json) {
        ...
        // convert polygon.buildings
        if (poly.hasOwnProperty('buildings') && !isEmpty(poly.buildings)) {
            if (poly.buildings.hasOwnProperty('clear')) delete poly.buildings['clear'];
            if (poly.buildings.hasOwnProperty('length')) delete poly.buildings['length'];

            let object = poly.buildings, array = [];
    ...
    function drawBuildings() {
        state.context().save();
        state.graphics.polygons.forEach(p => {
            if (!p.buildings || p.buildings === 0) return;
