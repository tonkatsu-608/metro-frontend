...
function Metro(canvas, cursorCanvas, data) {
    ...
    function Graphics(graphics) {
        ...
        this.cells = graphics ? graphics.cells.map(cell2Array) : this.diagram.cells;
        this.edges = graphics ? graphics.edges.map(edge2Array) : this.diagram.edges;
                        ...
                        if (subProperty === 'center') {
                            ...
                            for (let buildingProperty in buildingObject) {
                                ...
                                for (let subBuildingProperty in buildingObject[buildingProperty]) {
                                    ...
                                    if (subBuildingProperty !== 'length' && subBuildingProperty !== 'center' && subBuildingProperty !== 'color' && subBuildingProperty !== 'rotation' && subBuildingProperty !== 'size' && subBuildingProperty !== 'type' && subBuildingProperty !== 'bounds' && subBuildingProperty !== 'area') {
                                        ...
                                        if (subBuildingProperty === 'center') {
                                            ...
                                            subBuildingsArray.bounds = buildingObject[buildingProperty]['area'];
                                        }
                                    }
                                }
    ...
    function diagram2Array(diagram) {
        ...
        diagram.cells = diagram.cells.map(cell2Array);

        return diagram;
    }

    // convert Object(cell) to Array(cell)
    function cell2Array(c) {
        ...
        newCell.site.index = c.site.index;
        newCell.site.data = site2Array(c.site.data);
    ...
    function edge2Array(e) {
        ...
        edge.left.data = site2Array(e.left.data);

        if (e.right) {
            edge.right = [e.right.x, e.right.y];
            edge.right.index = e.right.index;
            edge.right.data = site2Array(e.right.data);
        }
        ...
        if (site.type === 'farm') {
            ...
            if (site.type === 'plaza') {
                ...
                if (polygon.subPolygons.length > 0) {
                    polygon.subPolygons = polygon.subPolygons.filter(sub => sub.area >= polygon.area / 25);
                }
            }
    ...
    function drawLine(context, p1, p2, width, color, isWaterLine) {
    ...
    function drawBuildings(context) {
        ...
        state.graphics.polygons.forEach(p => {
            ...
            p.subPolygons.forEach(subPoly => {
                if (!subPoly || subPoly === 0 || subPoly.length === 0 || !subPoly.center || !subPoly.buildings || !state.LAYERS.has(building)) return;
                drawSubPolygons(context, subPoly);

                subPoly.buildings.forEach(d => {
                    let k = ((state.N + 200) / 100) < 8 ? ((state.N + 200) / 100) : 8;
                    ...
                    geometries: cells.map(function (cell) {
                        ...
                        halfedges.forEach(function (halfedge) {
                            ...
                            if (edge.right) {
                                ...
                                cellArcs.push(site.index === edge.left.index ? i : ~i);
                                clipArc = null;
    ...
    function setWallTopology() {
        let topology = computeTopology(state.graphics.cells, state.graphics.edges);
