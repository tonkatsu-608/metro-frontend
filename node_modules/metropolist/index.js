...
function Metro(canvas, cursorCanvas, data) {
    ...
    function render() {
        ...
        /* draw docks */
        if (state.LAYERS.has(building)) drawDocks(state.context(), 'rgb(233, 233, 233)', 'rgb(233, 233, 233)', 5.5); // bgColor, edgeColor, lineWidth

        /* draw streetNames */
        if (state.LAYERS.has(building) && state.showStreetNames) drawStreetNames(state.context());
        ...
        state.graphics.polygons
            ...
            .forEach(p => {
                ...
                convert2Edges(p.edges)
                    ...
                    .forEach((e, i) => {
                        if (!e) return;
                        let streetName = state.STREET_NAMES[i % state.STREET_NAMES.length];
                        drawStreetName(context, e, streetName, 1.5, 'white');
                    });
                ...
                p.cutLines.forEach(cut => {
                    if (!cut) return;
                    drawStreetName(context, cut, cut[2], 1.5, 'lightgrey');
                })
            });
    ...
    function drawName(context, p1, p2, name, width, color) {
        ...
        context.fillStyle = color;
        context.strokeStyle = color;
        context.fillText(`${name}`, midX - fontWidth / 2, midY, fontWidth);
        context.fill();
        context.stroke();
        context.setTransform(1, 0, 0, 1, 0, 0);
        context.translate(state.transform.x, state.transform.y);
    ...
    function splitPolygon(type, poly, n, k) {
        ...
        while (subPoly.length < n) {
            ...
            let streetName = state.STREET_NAMES[~~(state.STREET_NAMES.length * Math.random())];
    ...
    function newGraphics(sites) {
        ...
        console.log("new graphics...", state.graphics);
