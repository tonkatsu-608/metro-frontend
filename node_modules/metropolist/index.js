...
function Metro(canvas, cursorCanvas, data) {
    ...
    var state = {
        ...
        ZOOM_FACTOR: { min: 1, max: 8 }, // zoom factor
    ...
    function Graphics(graphics) {
        const MIN_WIDTH = 20, MIN_HEIGHT = 20;
        const MAX_WIDTH = state.width() - 20;
        const MAX_HEIGHT = state.height() - 20;

        this.currentCastle = graphics ? graphics.currentCastle : null;
        this.sites = graphics ? graphics.sites.map(site2Array) : d3.range(state.N).map(() => [Math.random() * (MAX_WIDTH - MIN_WIDTH) + MIN_WIDTH, Math.random() * (MAX_HEIGHT - MIN_HEIGHT) + MIN_HEIGHT, 0]);
        this.voronoi = d3.voronoi().extent([[MIN_WIDTH, MIN_HEIGHT], [MAX_WIDTH, MAX_HEIGHT]]);
        // this.diagram = graphics ? diagram2Array(graphics.diagram) : this.voronoi(this.sites);
        ...
        if (poly.hasOwnProperty('subPolygons') && !isEmpty(poly.subPolygons)) {
            ...
            for (let property in object) {
                ...
                for (let subProperty in object[property]) {
                    ...
                    if (subProperty !== 'length' && subProperty !== 'center' && subProperty !== 'edges' && subProperty !== 'color' && subProperty !== 'rotation' && subProperty !== 'size' && subProperty !== 'type' && subProperty !== 'bounds' && subProperty !== 'area' && subProperty !== 'buildings') {
                        ...
                        } else if (subProperty === 'edges') {
                            let edgeObject = object[property]['edges'], edgeArray = [];
                            if (edgeObject.hasOwnProperty('clear')) delete edgeObject['clear'];
                            if (edgeObject.hasOwnProperty('length')) delete edgeObject['length'];

                            for (edgeProperty in edgeObject) {
                                let subEdgeArray = [];
                                if (edgeProperty !== 'length' && edgeProperty !== 'clear') {
                                    let p1 = [edgeObject[edgeProperty][0][0], edgeObject[edgeProperty][0][1]];
                                    let p2 = [edgeObject[edgeProperty][1][0], edgeObject[edgeProperty][1][1]];
                                    subEdgeArray.push(p1, p2);
                                }
                                edgeArray.push(subEdgeArray);
                            }
                            subArray.edges = edgeArray;
        ...
        if (poly.hasOwnProperty('farms') && !isEmpty(poly.farms)) {
            ...
            for (let property in object) {
                ...
                for (let subProperty in object[property]) {
                    ...
                    if (subProperty !== 'length' && subProperty !== 'center' && subProperty !== 'pattern' && subProperty !== 'rotation' && subProperty !== 'size' && subProperty !== 'type' && subProperty !== 'bounds' && subProperty !== 'area') {
                        if (object[property][subProperty][0] && object[property][subProperty][1]) {
                            subArray.push([object[property][subProperty][0], object[property][subProperty][1]]);
                        }
                    } else {
                        if (subProperty === 'center') {
                            ...
                            subArray.bounds = object[property]['area'];
                        }
                    }
                }
                array.push(subArray);
    ...
    function render() {
        ...
        // drawEdges(state.context(), 'black', 5);

        /* draw docks */
        if (state.LAYERS.has(building)) drawDocks(state.context(), 'rgb(233, 233, 233)', 'rgb(233, 233, 233)', 8.5); // bgColor, edgeColor, lineWidth

        /* draw walls */
        if (state.LAYERS.has(district)) drawWalls(state.context(), 'black', 8.5);

        /* draw street names of mainRoad/sideRoad  */
        drawStreetNames(state.context());
    ...
    function assignType4Site(index) {
        ...
        if (adjacentTypesMap.get('rich') >= 5) {
        ...
        // && Number(getAllTypes().get('religious')) <= 2.5 / 100 * state.N
        if (!s.isBoundary && s.type !== 'water') {
            if (Math.random() > 0.99) {
        ...
        if (site.type === 'farm') {
            ...
            if (site.type === 'plaza') {
                ...
                if (polygon.subPolygons.length > 0) {
                    polygon.subPolygons = polygon.subPolygons.filter(sub => sub.area >= polygon.area / 30);
                }
            }
    ...
    function makeBuildings(poly) {
        const BUILDINGS_NUMBER = {
            ...
            resultArr = splitPolygon(poly.type, poly, size, k);
     ...
     *
     * @TODO
     * find sites adjacent to s[`castle`],  s.wall = everySite.wall === 1 ? 0 : 1;
     */
    ...
    function drawLine(context, p1, p2, width, color, withoutCircle) {
        ...
        if (!withoutCircle) {
            ...
            context.arc(p1[0], p1[1], 5, 0, 2 * Math.PI, false);
        }
        context.moveTo(p1[0], p1[1]);
    ...
    // get all hashed edges of polygons/subPolygons
    function getAllHashedEdges() {
        let hashedEdges = new Set();

        state.graphics.polygons.forEach(p => {
            convert2Edges(p.edges).forEach(e => {
                if (e === null || e === undefined) return;
                hashedEdges.add(hashEdge([e[0], e[1]]));
                hashedEdges.add(hashEdge([e[1], e[0]]));
            });

            p.subPolygons.forEach(sp => {
                if (sp.edges === null || sp.edges === undefined) return;
                sp.edges.forEach(e => {
                    hashedEdges.add(hashEdge([e[0], e[1]]));
                    hashedEdges.add(hashEdge([e[1], e[0]]));
                });
    ...
    // get hash of point+slope
    function hashPointWithSlope(val1 = 0, val2 = 0) {
        return Math.floor(Number(val1 + val2) * 100) + '';
    }
            ...
            state.graphics.polygons
                ...
                .forEach(p => {
                    ...
                    convert2Edges(p.edges)
                        ...
                        .forEach(e => {
                            if (!e) return;
                            drawStreetName(context, e, 2.5, 'white', false);
                        });
                });
        }

        // then draw subPolygons'
        if (state.transform.k >= k * .95) {
            state.graphics.polygons.forEach(p => {
                if (!p.subPolygons) return;
                p.subPolygons.forEach(sp => {
                    if (!sp.edges) return;
                    sp.edges.forEach(e => {
                        if (!e) return;
                        drawStreetName(context, e, 2.5, 'red', true);
                    });
                });
            });
        }
    ...
    function drawStreetName(context, e, width, color, isSub) {
        const
            k = slope(e[0][0], e[0][1], e[1][0], e[1][1]),
            hashPoint1 = hashPointWithSlope(e[0][0], k),
            hashPoint2 = hashPointWithSlope(e[0][1], k),
            hashPoint3 = hashPointWithSlope(e[1][0], k),
            hashPoint4 = hashPointWithSlope(e[1][1], k);

        if (!isSub) {
            drawName(context, e[0], e[1], 'polygon st.', width, color);
            state.allEdges.add(hashPoint1);
            state.allEdges.add(hashPoint2);
            state.allEdges.add(hashPoint3);
            state.allEdges.add(hashPoint4);
        } else {
            if (state.allEdges.has(hashPoint1) || state.allEdges.has(hashPoint2) || state.allEdges.has(hashPoint3) || state.allEdges.has(hashPoint4)) {
                return;
            } else {
                drawName(context, e[0], e[1], 'sub poly st.', width, color);
                state.allEdges.add(hashPoint1);
                state.allEdges.add(hashPoint2);
                state.allEdges.add(hashPoint3);
                state.allEdges.add(hashPoint4);
        ...
        // let hashEdge1 = hashEdge([e[0], e[1]]);
        // let hashEdge2 = hashEdge([e[1], e[0]]);
        // if (state.allEdges.has(hashEdge1) || state.allEdges.has(hashEdge2)) {
        //     return;
        // } else {
        //     drawLine(context, e[0], e[1], width, color, true);
    ...
    function drawName(context, p1, p2, name, width, color) {
        ...
        // let theta = Math.atan2(midY, midX);
        // drawLine(context, e[0], e[1], width, color, true);
        context.beginPath();
        ...
        // context.arc(midX, midY, 1, 0, 2 * Math.PI, false);
        // context.moveTo(p1[0], p1[1]);
        // context.lineTo(p2[0], p2[1]);
        context.fillStyle = color;
        context.strokeStyle = color;
        context.fillText(`${name}`, midX - fontWidth / 2, midY + 1, fontWidth);
        context.fill();
        context.stroke();
        context.setTransform(1, 0, 0, 1, 0, 0);
        context.translate(state.transform.x, state.transform.y);
    ...
    function drawDistricts(context, p) {
        ...
        context.beginPath();
        // context.fillStyle = site.type === 'water' ? 'rgb(51, 102, 153)' : 'rgb(203, 197, 185)';
     ...
     * @param width
     */
    function drawContourLines(context, point, layer, color, width, isWaterLine) {
        context.save();
        context.translate(state.transform.x, state.transform.y);
        context.scale(state.transform.k, state.transform.k);
        state.graphics.triangles.forEach(triangle => {
            ...
            if (point >= vertices[0][layer] && point <= vertices[2][layer]) {
                ...
                drawLine(context, pt1, pt2, width, color, isWaterLine);
            }
        });
        context.restore();
    }
    ...
    function viewportInBbox(p1, p2, p3, p4) {
        const
            beyondWidth = p1.x < 0 && p3.x < 0 && p2.x > state.width() && p4.x > state.width(),
            beyondHeight = p1.y < 0 && p2.y < 0 && p3.y > state.height() && p4.y > state.height();
    ...
    function splitPolygon(type, poly, n, k) {
        ...
        while (subPoly.length < n) {
            ...
            if (type !== 'farm') {
                poly1.edges = getEdges(poly1);
                poly2.edges = getEdges(poly2);
            }
    ...
    // get edges of polygon/subPolygon
    function getEdges(vertices) {
        let edges = [];
        let hull = d3.polygonHull(vertices); // The returned hull is represented as an array containing a subset of the input points arranged in counterclockwise order.

        for (let i = 0; i < hull.length; i++) {
            let current = hull[i];
            let next = (i + 1) < hull.length ? hull[i + 1] : hull[0];
    ...
    function newGraphics(sites) {
        state.N = Math.abs(Number(sites)) > 500 ? 100 : Math.abs(Number(sites));
    ...
    function drawContourLine() {
        if (state.LAYERS.has(elevation)) {
            render();
            drawContourLines(state.context(), state.waterline, 'elevation', 'blue', 4.5, true);
            drawContourLines(state.context(), 0.25, 'elevation', 'red', 4.5, false);
            drawContourLines(state.context(), 0.5, 'elevation', 'green', 4.5, false);
            drawContourLines(state.context(), 0.75, 'elevation', 'yellow', 4.5, false);
        }
    }
