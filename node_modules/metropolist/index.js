...
function Metro(canvas, cursorCanvas, data) {
    ...
    function render() {
        ...
        /* draw street names of mainStreets/sideStreets  */
        if (state.LAYERS.has(building)) drawStreetNames(state.context());
        ...
        state.graphics.polygons
            ...
            .forEach(p => {
                ...
                p.cutLines.forEach(cut => {
                    if (!cut) return;
                    drawStreetName(context, cut, 2.5, 'red', true);
                })
            });
    ...
    function drawStreetName(context, st, width, color, isSub) {
        let hashEdge1 = hashEdge([st[0], st[1]]);
        let hashEdge2 = hashEdge([st[1], st[0]]);

        if (!isSub) {
            drawName(context, st[0], st[1], 'polygon st.', width, color);
            ...
            if (state.allEdges.has(hashEdge1) || state.allEdges.has(hashEdge2)) {
                ...
                drawName(context, st[0], st[1], 'sub poly st.', width, color);
                state.allEdges.add(hashEdge1);
                state.allEdges.add(hashEdge2);
            }
        }
    ...
    function splitPolygon(type, poly, n, k) {
        ...
        while (subPoly.length < n) {
            ...
            let cutLine = [[splitResult.cutLine.start.x, splitResult.cutLine.start.y], [splitResult.cutLine.end.x, splitResult.cutLine.end.y]];
